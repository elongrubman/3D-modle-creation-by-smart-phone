...
public:
    straight_line_equation(const Matx31f &directionVector, const Matx31f &point) : direction_vector(
...
//        waitKey(0);
    ...
    */
    ...
    /*!
     * this function gets coordinates in the grid axis system, and returns the voxel from the memory
     * that satisfies that the coordinates fall on its area, if no voxel is found we will push one. (this point is
     * certainly in the voxel area), if the coordinates fall in the area of two voxel (if one of the coordinates
     ...
     * @return - the voxel.
     */
    vector<voxel> getVoxelFromCoordinatesOrPush(float i, float j, float k, vector<tuple<int, int, int>>& keys) {
        ...
        if(fabs(i) > (EDGE_COORDINATE + 0.5) * step_size || fabs(j) > (EDGE_COORDINATE + 0.5) * step_size || fabs(k) > (EDGE_COORDINATE + 0.5) * step_size){
            ...
            vector<voxel> empty_voxel_vector;
            return empty_voxel_vector;
        }
...
//        vector<tuple<int, int, int>> keys;
        ...
        vector<voxel> voxels;
        returnVoxelsFromCoordinates(keys,voxels);
        return voxels;
    }
    ...
    void bresenhamAlgorithim(straight_line_equation line, tuple<int,int,int> entrance_voxel) {
        ...
        while (true) {
            if (dominant_direction == fabs(direction_x)) {
                ...
                auto vec = getVoxelFromCoordinatesOrPush(next_point_x, next_point_y, next_point_z, keys);
                if(vec.empty()){
                    break;
                }
    ...
    /*!
     * function that get vector of coordinates in the grid and returm the matching voxels
     * @param keys - vector<tuple>, the coordinates
     * @param voxels - vector<voxels>, the matching voxels
     */
    void returnVoxelsFromCoordinates(vector<tuple<int,int,int>>& keys, vector<voxel>& voxels) {
        for (int i = 0; i < keys.size(); ++i) {
            auto it = grid.find(keys[i]);
            /// in case the voxel does not exist, we create it and insert it into the gird, then returning it.
            if (it == grid.end()) {
                voxel inserted_voxel(1, 0);
                grid.insert(pair<tuple<int, int, int>, voxel>(keys[i], inserted_voxel));
                voxels.push_back(inserted_voxel);
            }
        }
    }
